...
class ReportDecorator < Draper::Decorator
  ...
  def failure_attempts
    object.report_executions.where('report_status IN(-1) OR send_status IN(-1)')
  end

  def expired?
    object.report_executions.where('(report_executions.report_status = 2 OR report_executions.send_status = 2)' \
